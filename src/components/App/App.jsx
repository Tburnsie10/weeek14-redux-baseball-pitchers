...
function App() {
  ...
  const pitcherList = useSelector((state) => state.pitchers);
  

  const catcherList = useSelector((state) => state.catchers)
  ...
  const handlePitcherSubmit = event => {
    ...
    dispatch({ type: `ADD-PITCHER`, payload: newPitcher });
    setNewPitcher('');
  };
  ...
  const handleCatcherSubmit = event => {
    ...
    dispatch({ type: `ADD-CATCHER`, payload: newCatcher });
    setNewCatcher('');
  };
